...
export type GetAiTopicsParams = { subjectId: number; page?: number; limit?: number }
...
export async function getAiTopics(params: GetAiTopicsParams): Promise<GetAiTopicsResponse> {
  ...
  const res = await fetch(`${AI_EXAM_BASE_URL}/api/topics?${sp.toString()}`, { method: 'GET', headers: getAuthHeaders() })
  const json = await res.json()
